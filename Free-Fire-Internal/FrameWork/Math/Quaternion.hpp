 ...
 */
#ifdef __has_include
#   if __has_include("../../FrameWork/Math/Vectors/Vector3.hpp")
...
struct Quaternion
{
    ...
    Vector3 ToEulerAngles() const {
        ...
        // Cálculo de ângulos de Euler a partir do Quaternion
        ...
        if (std::abs(sinp) >= 1)
            angles.Y = std::copysign(M_PI / 2.0f, sinp);  // Pitch
        else
    ...
    // Função para converter Quaternion para direção (unitária)
    Vector3 ToDirection() const {
        Vector3 direction;
        // Utiliza a fórmula para converter o Quaternion para a direção
    ...
    static inline Quaternion GetRotationToLocation(const Vector3& targetLocation, float y_bias, const Vector3& myLoc);
    ...
    friend std::ostream& operator<<(std::ostream& os, const Quaternion& quat) {
        os << XorStr("Quaternion(") << quat.X << ", " << quat.Y << ", " << quat.Z << ", " << quat.W << ")";
        return os;
    }
};
...
Quaternion Quaternion::GetRotationToLocation(const Vector3& targetLocation, float y_bias, const Vector3& myLoc) {
    Vector3 forwards = Vector3(targetLocation.X, targetLocation.Y + y_bias, targetLocation.Z) - myLoc;
    Vector3 upwards(0, 1, 0);  // O vetor 'up' padrão é (0, 1, 0)
...
Quaternion Quaternion::FromToRotation(Vector3 fromVector, Vector3 toVector)
{
    ...
    if (fabs(dot / k + 1) < 0.00001)
    {
        ...
        return Quaternion(Vector3::Normalize(ortho), 0);
    }
