...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
        ...
        for (int button = 0; button < 3; button++)
            ...
            {
                if (IsMouseClicked2(button, test_owner_id) && mouse_button_clicked == -1) { mouse_button_clicked = button; }
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    {
        size = ImVec2(ImGui::GetWindowSize().x - 20, 25);
    }
    else
    ...
    static std::map<ImGuiID, pfwaijfoiawfja> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, pfwaijfoiawfja() });
        it_anim = anim.find(id);
    }

    it_anim->second.a = ImLerp(it_anim->second.a, it_anim->second.c ? size.x : 0.f, g.IO.DeltaTime * 10.f);
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(188, 0, 255, 255).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
    ...
    {
        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, ImColor(14, 14, 14), 5);

        if (it_anim->second.a < size.x - 2)
        {
            ImGui::GetWindowDrawList()->AddCircleFilled(ImVec2(clickpos.x, clickpos.y), it_anim->second.a, ImColor(16, 16, 16, 255));
        }
        else
        {
            it_anim->second.c = false;
        }

        GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(it_anim->second.b), 5);
    }
    ImGui::PopClipRect();
...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    PushFont(FrameWork::Assets::InterSemiBold);
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(Label);
    const ImVec2 label_size = CalcTextSize(Label, NULL, true);
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(window->Size.x - 40, label_size.y + 25));
    ...
    static std::map<ImGuiID, adawfawf> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, adawfawf() });
        it_anim = anim.find(id);
    }

    it_anim->second.a = ImLerp(it_anim->second.a, Enabled ? ImColor(102, 120, 175, 30) : ImColor(102, 120, 175, 0), g.IO.DeltaTime * 10.f);
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(102, 120, 175, 0) : ImColor(102, 120, 175, 0), g.IO.DeltaTime * 10.f);

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(it_anim->second.b), 5);

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(it_anim->second.a), 5);


    if (Enabled)
    {
        PushFont(FrameWork::Assets::FontAwesomeSolid);
        window->DrawList->AddText(total_bb.Min + ImVec2(total_bb.GetWidth() / 2 - CalcTextSize(Icon).x / 2, total_bb.GetHeight() / 2 - CalcTextSize(Icon).y / 2), ImColor(1.f, 1.f, 1.f, 1.f), Icon);
        PopFont();
    ...
    {
        PushFont(FrameWork::Assets::FontAwesomeSolid);
        window->DrawList->AddText(total_bb.Min + ImVec2(total_bb.GetWidth() / 2 - CalcTextSize(Icon).x / 2, total_bb.GetHeight() / 2 - CalcTextSize(Icon).y / 2), ImColor(1.f, 1.f, 1.f, 0.4f), Icon);
        PopFont();
...
bool ImGui::KeyBind(const char* label, int* k)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    ImGui::PushFont(FrameWork::Assets::InterRegular14);
    ...
    static std::map<ImGuiID, KeyBinde> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, KeyBinde() });
        it_anim = anim.find(id);
    }

    it_anim->second.c = ImLerp(it_anim->second.c, *k ? 1.f : 0.f, g.IO.DeltaTime * 10.f);

    float size = (CalcTextSize(KeyNames[*k]).x + 10) * it_anim->second.c;
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(window->Size.x - 20 - size + offset_x, 0), window->DC.CursorPos + ImVec2(window->Size.x - 20 + offset_x, 23));
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(window->Size.x - 20 + offset_x, 23));
    ...
    it_anim->second.a = ImLerp(it_anim->second.a, (output || output2) ? ImColor(150, 150, 150) : ImColor(255, 255, 255), g.IO.DeltaTime * 10.f);
    //it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(102, 120, 175, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(255, 105, 180).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);

    ImGui::PushFont(FrameWork::Assets::InterRegular);

    const float label_offset_x = -3.0f; // Ajuste para mover a label 30 pixels para a esquerda

    window->DrawList->AddText(
        ImVec2(total_bb.Min.x + label_offset_x, frame_bb.Min.y + frame_bb.GetHeight() / 2 - CalcTextSize(label).y / 2),
        ImColor(255, 255, 255),
        label
    );
    ...
    ImGui::PushFont(FrameWork::Assets::InterRegular14);
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(12, 12, 12), 5);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.b), 5);

    PushClipRect(frame_bb.Min, frame_bb.Max, true);
    window->DrawList->AddText(ImVec2(frame_bb.Min.x + 5, frame_bb.Min.y + frame_bb.GetHeight() / 2 - CalcTextSize(label).y / 2), GetColorU32(it_anim->second.a), buf_display);
    PopClipRect();
    ImGui::PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    PushFont(FrameWork::Assets::InterRegular);
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(GetWindowSize().x - 20, 22));
    ...
    it_anim->second.move = ImLerp(it_anim->second.move, *v ? 1.f : 0.f, g.IO.DeltaTime * 10.f);
    it_anim->second.color = ImLerp(it_anim->second.color, *v ? ImColor(102, 120, 175) : ImColor(12, 12, 12), g.IO.DeltaTime * 10.f);
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(102, 120, 175, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);

    const ImRect check_bb(total_bb.Max - ImVec2(22, 22), total_bb.Max);

    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), GetColorU32(it_anim->second.color), 5);
   window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImGui::GetColorU32(it_anim->second.b), 5);

    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
    if (*v)
    {
        float checkmark_size = (square_sz - 10) * it_anim->second.move;
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(square_sz / 2, square_sz / 2) - ImVec2(checkmark_size / 2, checkmark_size / 2), ImColor(255, 255, 255), checkmark_size);
    }

    if (label_size.x > 0.0f)
        RenderText(ImVec2(total_bb.Min.x, total_bb.Min.y + total_bb.GetHeight() / 2 - label_size.y / 2), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items_count)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    PushFont(FrameWork::Assets::InterMedium);

    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    IM_ASSERT((flags & (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)) != (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)); // Can't use both flags together
   
    const float size = ImGui::GetWindowSize().x - 20;
    const float additionalWidth = 5.0f;
    const ImRect rect(window->DC.CursorPos + ImVec2(-3, 1), window->DC.CursorPos + ImVec2(size - 3 + additionalWidth, 45));
    const ImRect clickable(window->DC.CursorPos + ImVec2(-5, 20), window->DC.CursorPos + ImVec2(size - 4 + additionalWidth, 45));
    ...
    if (popup_open)
        it_anim->second.open_anim += 0.02f * (1.f - ImGui::GetIO().DeltaTime);
    else
        it_anim->second.open_anim -= 0.02f * (1.f - ImGui::GetIO().DeltaTime);
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(12, 12, 12), 4);

    window->DrawList->AddRect(clickable.Min, clickable.Max, GetColorU32(it_anim->second.b), 4);

    window->DrawList->AddText(rect.Min, ImColor(255, 255, 255), label);
    
    window->DrawList->AddText(clickable.Min + ImVec2(5, 12 - CalcTextSize(preview_value).y / 2), ImColor(186, 186, 186), preview_value);

    window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 13.f), 3.f + it_anim->second.active_circle_anim, ImColor(255, 105, 180));
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(items_count) * it_anim->second.open_anim));
    }
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(14, 14, 14, 255)));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    struct inputtext_state
    {
        ...
        float move_rect = 0.f, speed, value;
    };
    ...
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, hovered ? ImColor(102, 120, 175, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
    ...
    if (!is_multiline)
    {
        RenderNavHighlight(frame_bb, id);
        //window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(12, 12, 12), 5);
        //window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.rect_color), 5);
        ...
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.background), true, style.FrameRounding);
    }
    //if (!is_multiline)
    //{
    //    RenderNavHighlight(frame_bb, id);
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(5, frame_bb.GetHeight() / 2 - CalcTextSize(label).y / 2);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(settings::input::text_selected);
            float bg_offy_up = is_multiline ? 0.0f : -1.5f; 
            float bg_offy_dn = is_multiline ? 0.0f : 2.0f;
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(FrameWork::Assets::InterMedium, font_size, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        ...
        if (render_cursor)
        {
            ...
            if ((frame_bb.Min.x + it_anim->second.value) < cursor_screen_rect.Min.x) {
                it_anim->second.speed = (cursor_screen_rect.Min.x - (frame_bb.Min.x + it_anim->second.value)) / 15.f;
                ...
                it_anim->second.speed = ((frame_bb.Min.x + it_anim->second.value - cursor_screen_rect.Min.x)) / 15.f;
                it_anim->second.value -= it_anim->second.speed / ImGui::GetIO().Framerate * 200.f;
            }

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(frame_bb.Min + ImVec2(it_anim->second.value, 7), frame_bb.Min + ImVec2(it_anim->second.value, 23), GetColorU32(ImGuiCol_Text));

    /*        bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (state->CursorAnim <= 0.0f) || ImFmod(state->CursorAnim, 1.20f) <= 0.80f;
            ImVec2 cursor_screen_pos = ImTrunc(draw_pos + cursor_offset - draw_scroll);
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - g.FontSize + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f);*/

     /*       it_anim->second.input_move_one = ImLerp(it_anim->second.input_move_one, cursor_screen_rect.Min, g.IO.DeltaTime * 35.f);
            it_anim->second.input_move_two = ImLerp(it_anim->second.input_move_two, cursor_screen_rect.GetBL(), g.IO.DeltaTime * 35.f);*/

           //if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
           //     draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32(ImGuiCol_Text));

            //if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            //    draw_window->DrawList->AddLine(it_anim->second.input_move_one, it_anim->second.input_move_two, GetColorU32(ImGuiCol_Text));
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? settings::input::text_hint : settings::input::text);
            draw_window->DrawList->AddText(FrameWork::Assets::InterMedium, font_size, draw_pos, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        GetWindowDrawList()->AddText(
            GetWindowPos() + GetCursorPos() + ImVec2(-3.0f, square_sz / 2 - CalcTextSize(label).y / 2 - window->Scroll.y), // Move a label 30 pixels para a esquerda
            ImColor(255, 255, 255),
            label
        );
    }
    ...
    {
        window->DC.CursorPos = ImVec2(pos.x + GetWindowSize().x - 22 - square_sz, pos.y);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Render SV Square
    draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, 6.f);
    ...
    draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, bar_pos.y + 4), ImVec2(bar0_pos_x + bars_width, bar_pos.y + sv_bar_size - 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 6.f);

    float bar0_line_x = IM_ROUND(bar_pos.x + (1.f - H) * bars_width);

    bar0_line_x = ImClamp(bar0_line_x, picker_pos.x + 6.f, picker_pos.x + 207.f);

    it_anim->second.hue_bar = ImLerp(it_anim->second.hue_bar, bar0_line_x - bar_pos.x, g.IO.DeltaTime * 20.f);
    draw_list->AddCircleFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x, bar_pos.y + 6.9f), 5, GetColorU32(hue_color32), 100);
    ...
    draw_list->AddCircle(it_anim->second.circle_move + bar_pos + ImVec2(0, 1), it_anim->second.circle, IM_COL32(50, 50, 50, 150 * g.Style.Alpha), 32);
    draw_list->AddCircle(it_anim->second.circle_move + bar_pos, it_anim->second.circle, IM_COL32(230, 230, 230, 255 * g.Style.Alpha), 32);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 4), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 6.f);
        ...
        draw_list->AddCircle(ImVec2(it_anim->second.alpha_bar + bar_pos.x, bar1_bb.Min.y + 7.0f), 6, IM_COL32(255, 255, 255, 255), 100, 1.5f);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    PushFont(FrameWork::Assets::InterRegular14);
    ...
    window->DrawList->AddText(text_min + ImVec2(4, 1), ImColor(255, 255, 255, 105 + it_anim->second.active_text_anim + it_anim->second.hovered_text_anim), label);
