...
namespace ImGui
{
    ...
    //IMGUI_API bool          CustomChild(const char* str_id, const ImVec2& size, bool border = false);
    IMGUI_API bool          CustomChild(const char* str_id, const ImVec2& size);
    IMGUI_API bool          CustomChildLogin(const char* str_id, const ImVec2& size);
    ...
    ImU32 GetColorU322(const ImVec4& col, float alpha_mul);
    ...
    IMGUI_API bool          Tab(const char* Label, const char* Icon, bool Enabled);
    // square button with an arrow shape
    IMGUI_API bool          KeyBind(const char* label, int* k);
    ...
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags = 0, int items_count = -1);
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_AllowDuplicateId         = 1 << 6,   // false    // Allow submitting an item with the same identifier as an item already submitted this frame without triggering a warning tooltip if io.ConfigDebugHighlightIdConflicts is set.
};
